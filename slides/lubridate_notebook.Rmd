...
2) know some introductory level techniques for handling date-time data in R using the `lubridate` package. 
...
The `lubridate` package contains a suite of functions `ymd`, `ydm`, `mdy`, `myd`, `dmy`, and `dym` that can be used ot parse a string or character into a date. For example,
...
```{r}
food_services %>% mutate(year=year(Date),month=month(Date),day=day(Date)) %>% group_by(year) %>% summarise(mean_growth=mean(`NSA Employees Growth`))
```
...
Applying on of the functions `make_date` or `make_datetime` across multiple columns allows us to combine the individual components:
