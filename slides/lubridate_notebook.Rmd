...
Data comes in a variety of types. For example, we measure quantities numerically with decimal numbers, count certain observations with whole numbers, and record other observations such as names with characters or words. In R, these date are represented with specific types. For example, numeric values are represented with type `numeric`, while many types of nonnumeric data are represented with type `character`. Here are some examples where we show how to determine how R has represented the data:
...
Often, we store a data set as a `data.frame` or `tibble` in R. Each column in a data frame can have a different type for the data it contains. Let's read in some sampe data to see this. We will work with two data sets:
